...
1. Maintaining key YANG data models, which include at least the following: VPN Common Model, L2VPN Service Model (L2SM), L3VPN Service Model (L3SM), L2VPN Network Model (L2NM), L3VPN Network Model (L3NM), Attachment Circuit Common Model, Attachment Circuit-as-a-Service (ACaaS), Attachment Circuit Network Model, Service Attachment Points (SAP), Network Slicing, Service Assurance for Intent-Based Networking (SAIN), Virtual Network (VN), Abstraction and Control of TE Networks (ACTN), and the YANG Data Model for Network and VPN Service Performance Monitoring. Maintenance activities may include augmentations, new revisions, and refactoring or restructuring modules to address identified operational issues.
1. Develop reusable abstractions and a common foundation for service YANG models that allow multiple IETF service models to be built on a shared extensible foundation.

The WG may develop new service or network YANG models related to IETF technologies when gaps are identified.
...
1. Develop a reusable framework for service YANG models.
